 ...
 *
 * This does the data encoding over the complex numbers, using `i` as the primitive fourth root of 1. This
 * gives the powers of roots of unity we use as [1, i, -1, -i].
 *
 * Notes:
 *  - It is hardcoded to deal with only 4 samples (two data elements!), up to two of which can be lost.
 *  - This is not implemented over a finite field as per Eth2, but over the complex integers. See finite.c for a toy
